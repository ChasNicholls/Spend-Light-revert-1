...
// --- flexible keyword matcher: supports multi-word keywords in order, allowing punctuation/extra tokens between
function matchesKeyword(descLower, keywordLower) {
  if (!keywordLower) return false;
  const parts = String(keywordLower).trim().split(/\s+/).filter(Boolean);
  if (parts.length === 0) return false;
  let pos = 0;
  for (const part of parts) {
    const idx = descLower.indexOf(part, pos);
